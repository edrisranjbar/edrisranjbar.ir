            ...
            <ul class="list-group mx-0 px-0 w-100">
                ...
                <li class="list-group-item">
                    <form action="{{ route('comments.destroy', $comment->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="d-inline-block mt-1 float-left me-1 badge
                            border-0 text-bg-danger p-2 clickable-badge">
                            حذف
                        </button>
                    </form>
                    <strong>{{ $comment->author->name }}:</strong>
                    <small class="d-inline-block mt-1 badge badge-pill badge-secondary">
                        {{ $comment->updatedAgo }}
                    </small>
                    ...
                    <div class="d-flex align-items-center gap-2">
                        <a href="{{ url('admin/comments/' . $comment->id . '/toggleApprovementStatus') }}">
                            <span
                                class="badge @if ($comment->confirmed) text-bg-success @else text-bg-secondary @endif p-2 cursor-pointer clickable-badge">
                        ...
                        <button class="btn btn-link text-danger" data-bs-toggle="modal"
                            data-bs-target="#delete-comment-reply-modal" data-comment-id="{{ $comment->id }}">
                            حذف پاسخ
                        </button>
                        @endif
                    </div>
