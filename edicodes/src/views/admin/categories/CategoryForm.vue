...
async function fetchCategory(id) {
  try {
    const response = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/admin/categories/${id}`, {
      headers: {
  ...
  try {
    ...
    if (isEditing.value) {
      ...
      response = await axios.put(
        `${import.meta.env.VITE_API_BASE_URL}/admin/categories/${route.params.id}`, 
        categoryData,
        {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('admin_token')}`
